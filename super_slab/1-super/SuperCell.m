...
so  = ftn58sparse.isSO;
abc = ftn58sparse.abc;
orb = ftn58sparse.Orbitps;      %! 1-45,1-45
orb(:,2) = [1:size(orb,1)]';    %! 1-90
...
for ii=1:size(ps, 1)
    ps(ii, 5:7) = ps(ii,1:3)*BR/(NBR);    %! 5-7 not cartesian because *BR/NBR
end
ps(:,8) = [1:size(ps,1)]';                %! 1-15 for atome id
%%%--- new vector grid
%%%--- ps1 is for a table for atoms in a 3D grid
nbound_i = vec1(1);
nbound_j = vec2(2);
nbound_k = vec3(3);
nx = -nbound_i:nbound_i;
ny = -nbound_j:nbound_j;
nz = -nbound_k:nbound_k;
[tty,ttx,ttz]=meshgrid(ny,nx,nz);     %! supercell 3*3*5
nof=[ttx(:),tty(:),ttz(:)];           %! 3*3*5mesh--->row

pstmp = [ps(:,1:3) ps(:,4) ps(:,8)];
pstmp2=repmat(pstmp,size(nof,1),1);   %! 45*15,5
nof2 = [];

for tt = 1:size(nof,1)
    nof2 = [nof2;repmat(nof(tt,:),size(pstmp,1),1)];   %! for each cell a1a2a3 repmat atom number
end
ps1 = pstmp2;                                      %! assign array size
ps1(:,1:3) = pstmp2(:,1:3)+nof2;                   %! atom pos in each cell
for ii=1:size(ps1, 1)
    ps1(ii,6:8)  = ps1(ii,1:3)*BR;                 %! form lattice to cartesian
    ps1(ii,9:11) = ps1(ii,6:8)/(NBR);              %! form cartesian to lattice in supercell
end
