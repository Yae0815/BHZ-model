...
ijs = zeros(length(dd)*3,2);
tts = zeros(length(dd)*3,1);
dds = zeros(length(dd)*3,3);
...
%% ----------  A/B layer scaling  ---------- %%
scaleAB   = 1.02;               % >1 代表 A(odd) 層放大
norb0     = size(orbitps,1);    % 原單胞中 total # orbitals
layer_i   = ceil( SPftn58sparse.ij(:,1) / norb0 );   % 每筆 hopping 左端軌域所屬層
sameCell  = SPftn58sparse.ij(:,2) <= norb0;          % 只改 intra-cell hopping；若要連 interlayer 也改請拿掉這行判斷

oddMask   =  mod(layer_i,2)==1  &  sameCell;         % 奇數層 = A
evenMask  =  mod(layer_i,2)==0  &  sameCell;         % 偶數層 = B

SPftn58sparse.tt(oddMask)  = SPftn58sparse.tt(oddMask)  *  scaleAB;
SPftn58sparse.tt(evenMask) = SPftn58sparse.tt(evenMask) /  scaleAB; % 相當於 ×0.98
