...
nk = 100;
p1 = [linspace(1.0,0.0,nk+1)' linspace(0.0,0.0,nk+1)'];
p2 = [linspace(0.0,2/3,nk)' linspace(0.0,2/3,nk)'];
...
function cplot(x,y,c,ls,varargin)
    % 以插值方式讓線段顏色隨 c 變化
    surface([x(:) x(:)]', [y(:) y(:)]', zeros(2,numel(x)), [c(:) c(:)]', ...
            'FaceColor','none','EdgeColor','interp','LineStyle',ls, varargin{:});
end
