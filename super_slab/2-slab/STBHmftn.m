...
load('slab_info.mat');
%%% ------------------- %%%
% 在 load('slab_info.mat'); 之後加：
dir  = xyz_dir;     % 1->x, 2->y, 3->z（你在 SlabCell 存進來的）
col  = 3 + dir;     % orbitps 中 4:6 對應 S1,S2,S3 -> 欄位 4/5/6
...
ijs =zeros(size(ib1,1)*3,2);
tts =zeros(size(ib1,1)*3,1);
dds =zeros(size(ib1,1)*3,3);
...
%             index = find((abs(orbitps(1:end,6)-component(3))<0.001)&(orbitps(:,2)==ib2(temp1(jj))));
               tol = 1e-3;
               index = find( (abs(orbitps(:,col) - component(dir)) < tol) & ...
              (orbitps(:,2) == ib2(temp1(jj))) );    % 6,3 : z direction
